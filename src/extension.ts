...
import { ALL_FORMATTINGS, getAllForAutoComplete } from "./minecraftFormatting";

const FORMAT_REGEX = /(\u00a7([0-9a-fk-orA-FK-OR]))/g;

const AUTO_COMPLETE_MAP = getAllForAutoComplete();
...
const FORMAT_DECORATIONS = new Map<string, vscode.TextEditorDecorationType>();

for (const formatting of ALL_FORMATTINGS) {
...
const updateEditorDecorations = (editor: vscode.TextEditor) => {
  ...
  const equalsRanges: vscode.Range[] = [];
  const newlineEscapeRanges: vscode.Range[] = [];
  const formatRanges = new Map<string, vscode.Range[]>();
  ...
  for (let lineIndex = 0; lineIndex < document.lineCount; lineIndex++) {
    ...
    if (equalsIndex !== -1) {
      ...
      if (keyEndOffset > keyStartOffset) {
        keyRanges.push(
          new vscode.Range(document.positionAt(keyStartOffset), document.positionAt(keyEndOffset))
        );
      }

      equalsRanges.push(
        new vscode.Range(
          document.positionAt(lineStartOffset + equalsIndex),
          document.positionAt(lineStartOffset + equalsIndex + 1)
        )
      );
...
const checkAndReplaceLastWord = async (
  ...
  await editor.edit((editBuilder) => {
    editBuilder.replace(
      new vscode.Range(line.range.start.line, wordStartOffset, line.range.start.line, wordEndOffset),
      code
    );
  });
...
const isLikelyLangKey = (text: string): boolean => {
  return text.includes(".") || text.includes("_") || text.includes("-");
};

const getStringLiteralAtPosition = (
  document: vscode.TextDocument,
  ...
  const scanQuote = (quote: string): { start: number; end: number } | null => {
    let start = -1;
  ...
  const range = new vscode.Range(
    new vscode.Position(position.line, match.start + 1),
...
const findKeyInLine = (
  line: string,
  searchKey: string
): { matchType: number; matchedKey: string } | null => {
...
const provideDefinition = async (
  ...
  if (isLangFile) {
    ...
    const matches: { priority: number; location: vscode.Location }[] = [];
    ...
    for (const file of filesToSearch) {
      ...
      for (let i = 0; i < lines.length; i++) {
        ...
        const location = new vscode.Location(
          file,
          new vscode.Range(new vscode.Position(i, matchIndex), new vscode.Position(i, matchIndex))
        );
  ...
  for (const file of langFiles) {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      const location = new vscode.Location(
        file,
        new vscode.Range(new vscode.Position(i, 0), new vscode.Position(i, 0))
      );
